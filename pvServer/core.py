...
def create_login_response(userData):
    ...
    refreshToken = createRefreshToken(username, config.REFRESH_COOKIE_MAX_AGE_SECS)
    accessToken = createAccessToken(username, config.ACCESS_TOKEN_MAX_AGE_SECS, roles)
    d = {
        "login": True,
        "username": username,
        "roles": roles,
    ...
    if config.SECURE:
        resp.set_cookie(
            key="refreshToken",
            value=refreshToken,
            max_age=config.REFRESH_COOKIE_MAX_AGE_SECS,
            secure=True,
            httponly=True,
            samesite=None,
        )
...
def login_local():
    if not config.REACT_APP_DisableStandardLogin:
...
def login_ldap():
    ...
    if config.REACT_APP_EnableActiveDirectoryLogin:
        user = request.json.get("user", None)
...
def login_google():
    if config.REACT_APP_EnableGoogleLogin:
...
def remove_pv_connection(message):
    global clientPVlist, myuid
    pvname1 = str(message["pvname"])
    authenticated = False
    if config.REACT_APP_DisableLogin:
        authenticated = True
        accessControl = {
            "userAuthorised": True,
            "permissions": {"read": True, "write": True},
...
def db_remove_watch(message):
    global clientPVlist, myuid, thread_lock
    dbURL = str(message["dbURL"])
    if config.REACT_APP_DisableLogin:
        accessControl = {
            "userAuthorised": True,
...
def db_read_watch_broadcast(message):
    global clientPVlist, clientDbWatchList, myDbWatchUid, thread_lock
    dbURL = str(message["dbURL"])
    authenticated = False
    if config.REACT_APP_DisableLogin:
        authenticated = True
...
def admin_add_user(message):
    global clientPVlist, clientDbWatchList, myDbWatchUid
    isAdmin = checkIfAdmin(message["clientAuthorisation"])
    if isAdmin:
        try:
            client = open_mongo_db_client("ADMIN_DATABASE", "rasAdminDb")
...
def authorize_client(message):
    if not config.REACT_APP_DisableLogin:
