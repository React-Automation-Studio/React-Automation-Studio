...
const AlarmSetup = (props) => {
  ...
  const isAlarmUser = useMemo(() => {
    const isLoggedIn = username !== undefined;
    const alarmUser = isLoggedIn ? roles.includes("alarmUser") : false;
  ...
  const [alarmLogSearchStringStore, setAlarmLogSearchStringStore] =
    useState("");
  ...
  const [alarmTableSearchStringStore, setAlarmTableSearchStringStore] =
    useState("");
  ...
  const alarmPVDictReducer = useCallback(
    (state, action) => {
      switch (action.type) {
        case "updatePVData":
          if (
            action.pvData.initialized &&
            (!loadAlarmTable.alarmPV || backdropOpen)
          ) {
  ...
  const areaPVDictReducer = useCallback(
    (state, action) => {
      switch (action.type) {
        case "updatePVData":
  ...
  const dbPVDataRaw = useMongoDbWatch({
  ...
  useEffect(() => {
    if (dbPVDataRaw !== null) {
      ...
      dbPVDataRaw.map((area) => {
        ...
        const areaHasRoles = area?.roles ? area.roles.length > 0 : false;
          ...
          Object.keys(area).map((areaKey) => {
            if (areaKey === "pvs") {
              ...
              const items = Object.entries(area[areaKey]).map((value) => {
                localLastPVKey[area["area"]] = parseInt(
                  value[0].replace("pv", "")
                );
                return value;
              });
              ...
              if (subAreaMatchesRole) {
                areaSubAreaMongoId[textEval] =
                  areaKey;
                areaMongoId[textEval] =
                  area["_id"]["$oid"];
                // Area enabled for subArea includes parent area
                localAreaEnabled[textEval] =
                  area[areaKey]["enable"] && localAreaEnabled[area["area"]];
                // Backwards compatible
                localAreaBridged[textEval] = {
                  bridge: area[areaKey]["bridge"] ?? false,
                  bridgeTime: area[areaKey]["bridgeTime"] ?? "",
                };
                localLastArea = textEval;
                if (localAreaNames[areaNamesIndex]["subAreas"]) {
                  localAreaNames[areaNamesIndex]["subAreas"].push(
                    area[areaKey]["name"]
                  );
                } else {
                  localAreaNames[areaNamesIndex]["subAreas"] = [
                    area[areaKey]["name"],
                  ];
                }
                // map all alarms in subArea
                const items = Object.entries(area[areaKey]["pvs"]).map(
                  (value) => {
                    localLastPVKey[textEval] =
                      parseInt(value[0].replace("pv", ""));
                    return value;
                  }
                );
                items.sort((A, B) => {
                ...
                items.map((item) => {
                  localAreaAlarms.push([
                    `${area["area"]}=${area[areaKey]["name"]}=${item[0]}`,
                    item[1],
                  ]);
                  localLastAlarm = item[1]["name"];
                  return null;
  ...
  const handleDisableEnableGlobal = useCallback(
    (value) => {
      // console.log(value)
  ...
  const handleSearchAlarmTable = useCallback(
    (event) => {
  ...
  const handleSearchAlarmLog = useCallback(
    (event) => {
      const srch = event.target.value;
  ...
  const handleAckAllAreaAlarms = useCallback(
    (event, index) => {
  ...
  const handleTableItemCheck = useCallback(
    (event, index, alarm, field, value) => {
      ...
      if (index.includes("=")) {
        const subAreaId =
          areaSubAreaMongoId[index] + ".pvs." + alarm + "." + field;
  ...
  const handleTableEnableCheck = useCallback(
    (event, areaName, alarm, entryIndex) => {
      // to prevent re render of alarm log table?
      event.preventDefault();
      event.stopPropagation();
  ...
  const handleExecuteEnablePV = useCallback(() => {
    ...
    if (index.includes("=")) {
      ...
      newvalues = {
        $set: {
          [areaSubAreaMongoId[index] + ".pvs." + alarm + ".bridge"]:
            enableDialogData.bridge,
          [areaSubAreaMongoId[index] + ".pvs." + alarm + ".bridgeTime"]:
            enableDialogData.bridgeTime,
          [areaSubAreaMongoId[index] + ".pvs." + alarm + ".enable"]:
            enableDialogData.enable,
        },
      };
      dbUpdateOne({
        dbURL: `mongodb://ALARM_DATABASE:${props.dbName}:pvs`,
  ...
  const handleTableItemRightClick = useCallback(
    (event, index, entryIndex) => {
      ...
      if (
        areaEnabled[areaName] &&
        filteredAreaAlarms[entryIndex][1]["enable"]
      ) {
  ...
  const handleAlarmAcknowledge = useCallback(
    (event, index) => {
  ...
  const handleEnableDisableArea = useCallback(
    (event, index, value) => {
      event.preventDefault();
      event.stopPropagation();
  ...
  const handleExecuteEnableDisableArea = useCallback(
    (event) => {
      ...
      if (index.includes("=")) {
        ...
        newvalues = {
          $set: {
            ...
            [areaSubAreaMongoId[index] + ".bridgeTime"]:
              enableDialogData.bridgeTime,
  ...
  const handleListItemClick = useCallback(
    (event, index) => {
      ...
      if (localAreaSelectedName.length > 1) {
        // selected area is a subArea
        localAreaSelectedName =
          localAreaSelectedName[0] + " > " + localAreaSelectedName[1];
      } else {
        // selected area is an area
        if (index === areaSelectedIndex) {
          // selected same area twice
          setAreaSubAreaOpen({
            [index]: !areaSubAreaOpen[index],
          });
        } else {
          // selected a different area
          setAreaSubAreaOpen({
            [index]: true,
          });
        }
  ...
  const handleAddNewPV = useCallback(
    (event, index) => {
      const lastKey = lastPVKey[index];
      const newKey = isNaN(lastKey) ? 0 : lastKey + 1;
  ...
  const handleAddNewSubArea = useCallback(
    (event, index) => {
      setAreaContextOpen({});
      setAlarmAdminListExpand(false);
      setAddSubAreaData({
        edit: false,
        areaIndex: index,
        areaNextSubAreaKey: isNaN(lastSubAreaKey[index])
          ? 0
          : lastSubAreaKey[index] + 1,
        subArea: "",
        addRoles: false,
        roles: [],
      });
  ...
  const handleDeleteArea = useCallback((event, index) => {
    const areaName = index.includes("=") ? index.split("=")[1] : index;
  ...
  const handlePopNewPVInfo = useCallback(
    (index) => {
      const newPvs = newPVInfo.pvs.filter(
  ...
  const handleUpdateNewPVInfoPVName = useCallback(
    (event, index) => {
  ...
  useEffect(() => {
    const keyedAreaAlarms = areaAlarms.reduce((acc, entry) => {
      ...
      if (!areaSelectedIndex.includes("=")) {
        // areaSelectedIndex is area
    ...
    const searchedAreaAlarms = keyedAreaAlarms.reduce((acc, entry) => {
      const value = entry[1];
      const visible = value["name"]
        .toLowerCase()
        .includes(alarmTableSearchString.toLowerCase());
      if (visible) {
        acc.push(entry);
      }
      return acc;
      ...
      areaAlarms.map((entry) => {
        ...
        const pvObject = (
          <PV
            ...
            pvData={(pvData) =>
              dispatchAlarmPVDict({ type: "updatePVData", pvData: pvData })
            }
          />
        );
      ...
      Object.keys(areaEnabled).map((areaName) => {
        const pvObject = (
          <PV
            ...
            pvData={(pvData) =>
              dispatchAreaPVDict({ type: "updatePVData", pvData: pvData })
            }
          />
        );
  ...
  useEffect(() => {
    ...
    alarmLogExpand,
    alarmLogIsExpanded,
    alarmTableIsExpanded,
  ]);

  const setDialogPvData = useCallback(
    (pvData) => {
      const epicsPVName = pvData.pvName?.replace("pva://", "");
      const index = newPVInfo.pvs.findIndex(
        (item) => item.pvname === epicsPVName
      );
  ...
  useEffect(() => {
    ...
    if (backdropOpen) {
      timer1 = setInterval(() => {
        setASRestartProgress((prevProgress) =>
  ...
  return (
    <div>
      ...
      <EnableDialog
        ...
        executeEnable={
          enableDialogData.type === "pv"
            ? handleExecuteEnablePV
            : handleExecuteEnableDisableArea
        }
              ...
              >
                ...
                >
                  <Grid item xs={2} style={{ textAlign: "right" }}>
                    ...
                    {isAlarmUser && (
                      <Menu
                        ...
                        anchorPosition={
                          contextMouseY !== null && contextMouseX !== null
                            ? { top: contextMouseY, left: contextMouseX }
                            : null
                        }
                      >
                        ...
                        <MenuItem onClick={handleAckGlobal}>
                          <ListItemIcon>
                            <DoneAllIcon fontSize="small" />
                          </ListItemIcon>
                          <Typography variant="inherit">
                            ACK ALL AREAS' alarms
                          </Typography>
                        </MenuItem>
                        {isAlarmAdmin && (
                          <MenuItem
                            onClick={() =>
                              setAlarmAdminGListExpand(!alarmAdminGListExpand)
                            }
                          >
                            <ListItemIcon>
                            ...
                            {alarmAdminGListExpand ? (
                              <ExpandLess style={{ marginLeft: 16 }} />
                            ) : (
                              <ExpandMore style={{ marginLeft: 16 }} />
                            )}
                          </MenuItem>
                        )}
                        <Collapse
                          in={alarmAdminGListExpand}
                          timeout="auto"
                          unmountOnExit
                        >
                  ...
                  <Grid item xs={10}>
                    ...
                    >{`ALARM AREAS ${enableAllAreas ? "" : "[DISABLED]"}`}</div>
                  </Grid>
                  <Grid item xs={12}>
                    {areaNames ? (
                      <AlarmList
