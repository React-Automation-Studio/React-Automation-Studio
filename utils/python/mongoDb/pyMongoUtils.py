...
def open_mongo_db_client(hostDatabaseName, databaseName):
    try:
    ...
    try:
        if mongoAuth:
            client = MongoClient(
                "mongodb://%s:%s@%s"
                % (MONGO_ROOT_USERNAME, MONGO_ROOT_PASSWORD, DATABASE),
                replicaSet=DATABASE_REPLICA_SET_NAME,
            )
            # Wait for MongoClient to discover the whole replica set and identify MASTER!
            sleep(0.1)
        else:
            client = MongoClient(
                "mongodb://%s" % (DATABASE), replicaSet=DATABASE_REPLICA_SET_NAME
            )
            # Wait for MongoClient to discover the whole replica set and identify MASTER!
            sleep(0.1)
        if databaseName is None:
            # Do not check for valid database
            return client
        else:
