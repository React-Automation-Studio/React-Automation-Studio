...
import ReactAutomationStudioContext from '../SystemComponents/AutomationStudioContext';
...
const useServerStatistics=(props)=>{
  ...
  useEffect(() => {
    const  updateServerStatistics = (msg) => {
      setStatistics(msg);
    console.log(msg)
    }
    const getServerStatistics=()=>{
      ...
      if(socket){
        socketRef.current.emit('request_server_statistics', { 'clientAuthorisation': jwtRef.current });
        
      }
    }
    const timer =setInterval(getServerStatistics,5000)
    const reconnect = () => {
      if (props.debug) {
        console.log('ServerStatistics, client: reconnect');
      }
      socketRef.current.emit('request_server_statistics', { 'clientAuthorisation': jwtRef.current });
    }
    ...
    if(socket){
      ...
      socketRef.current.on("server_statistics", updateServerStatistics);
    ...
    return () => {
      ...
      if(socket){
        ...
        socketRef.current.removeListener("server_statistics", updateServerStatistics);
      }
...
const ServerStatistics =(props)=> {
  const statistics=useServerStatistics({});
...
export default withWidth()(withStyles(styles,{withTheme:true})(ServerStatistics));
