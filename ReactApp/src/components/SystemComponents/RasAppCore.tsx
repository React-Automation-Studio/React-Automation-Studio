...
class RasAppCore extends Component<RasAppCoreProps, RasAppCoreState> {
  constructor(props) {
    ...
    this.setRefreshTokenConfig = (config) => {
      ...
      const timer = setTimeout(
        ...
        system.refreshTokenConfig.refreshTimeout * 1000
      );
    ...
    let refreshTokenConfig = JSON.parse(
      localStorage.getItem("refreshTokenConfig")
    );
  ...
  handleClientAuthorisation(msg) {
    ...
    this.setState({
      Authorised: msg.successful,
      AuthorisationFailed: msg.successful !== true,
    });
  }
  ...
  getRefreshToken() {
    ...
    if (useCookie === true) {
      ...
      console.warn(
        "React Automation Studio SECURE mode is not in use, and as a consequence the refresh token will stored in local storage. This mode should not be used in production, see styleguide for correct implementation"
      );
      const { refreshToken } = this.state.system.refreshTokenConfig;
      body = JSON.stringify({ refreshToken: refreshToken });
    }
    axios
      .post("/api/refresh", body, options)
      .then((response) => {
        // handle success
        const { data } = response;
  ...
  componentWillUnmount() {
    ...
    socket.removeListener(
      "clientAuthorisation",
      this.handleClientAuthorisation
    );
